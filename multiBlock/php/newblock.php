...
<form method="POST" action="<?php echo DOMAIN;?>/admin/plugin/multiblock?newblock=&newmulticategory=<?php echo @$_GET['newmulticategory'];?>&choosecat"  class="mbformer">
<input type="hidden" id="jstokenCSRF" name="tokenCSRF" value="<?php echo $tokenCSRF;?>">

<div style="background:#fafafa;border:solid 1px #ddd; padding:10px;">
<?php echo $L->get('section');?>: <input
        type="text"
        name="cat"
        class="cat"
        disabled="disabled"
...
foreach (glob(PATH_ROOT."/bl-content/uploads/pages/*/*.{jpg,png,gif,bmp,jpeg,webp}",GLOB_BRACE) as $images) {
     ...
     $newurl = str_replace(PATH_ROOT, DOMAIN , $images);
   ...
   $roter = str_replace(PATH_ROOT, DOMAIN , $path_parts['dirname']).'/thumbnails/'.$path_parts['basename'];
     ...
     <p>'.str_replace(DOMAIN.'/bl-content/uploads/','',$newurl).'</p>
