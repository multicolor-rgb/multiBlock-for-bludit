...
';
    ...
    foreach (glob(PATH_CONTENT."/multiBlock/".str_replace(" ","-",$_GET['newmulticategory'])."/*.json") as $filename) {
        ...
        <a href="'.DOMAIN.'/admin/plugin/multiblock?newmulticategory='.str_replace(" ","-",$_GET['newmulticategory']).'&createblock=&namefile='.$name.'" class="mb_edit btn btn-sm btn-dark" style="width:45px;">'.$L->get("edit").'</a>
 ...
 if(isset($_GET['newmulticategory'])){
    ...
    <form method="post" style="margin-top:20px;" action="'.DOMAIN.'/admin/plugin/multiblock?newblock&newmulticategory='.$_GET['newmulticategory'].'">
...
if(isset($_GET['choosecat'])){
   ...
   echo "document.querySelector('.addnewblock').setAttribute('href','".DOMAIN."/admin/plugin/multiblock?newblock&newmulticategory=".@$_GET['newmulticategory']."')";
